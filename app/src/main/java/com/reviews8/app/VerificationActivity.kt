    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn.setOnClickListener {
            ...
            if (mode == "forgot") {
                ...
                if (newPass.length < 6) { tvResult.text = "كلمة المرور قصيرة"; return@setOnClickListener }
                if (newPass != confirmNew) { tvResult.text = "إعادة غير متطابقة"; return@setOnClickListener }
                ...
                scope.launch {
                    try {
                        ...
                        if (res.status == "ok") { Toast.makeText(this@VerificationActivity, "تم تغيير الباسورد", Toast.LENGTH_LONG).show(); finish() }
                ...
                scope.launch {
                    try {
                        ...
                        if (verify.status == "ok") {
                            ...
                            if (reg.status == "ok") { Toast.makeText(this@VerificationActivity, "تم إنشاء الحساب", Toast.LENGTH_LONG).show(); finish() }
