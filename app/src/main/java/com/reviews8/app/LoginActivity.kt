    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn.setOnClickListener {
            ...
            if (email.isEmpty() || pass.isEmpty()) {
                tv.text = "اكمل البيانات"
                return@setOnClickListener
            }
            tv.text = "جاري تسجيل الدخول..."
            scope.launch {
                try {
                    val res = withContext(Dispatchers.IO) {
                        RetrofitClient.api.login(LoginRequest(email = email, password = pass))
                    }
                    if (res.status == "ok") {
                        ...
                        tv.text = "تم بنجاح: ${res.user?.email}"
                    } else {
                        tv.text = res.msg ?: "خطأ في الدخول"
                    }
                } catch (e: Exception) {
                    tv.text = "خطأ اتصال: ${e.message}"
                }
            }
        }
    }
